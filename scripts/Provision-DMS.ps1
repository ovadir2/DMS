...
try {
    ...
    if (-not $SkipSiteCreation) {
        foreach ($s in @(
            @{ Url = $DcUrl; Title = (T 'Documents Management System (DMS)' 'מערכת לניהול ושיתוף קבצים (DMS)') },
    ...
    foreach ($site in @(@{ Key = 'DC'; Url = $DcUrl }, @{ Key = 'EX'; Url = $ExUrl })) {
        ...
        $siteTitle = if ($site.Key -eq 'DC') { T 'Documents Management System (DMS)' 'מערכת לניהול ושיתוף קבצים (DMS)' } else { T 'DMS - Large File Exchange' 'DMS - העברת קבצים גדולים' }
